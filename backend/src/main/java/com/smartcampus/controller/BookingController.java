...
import com.smartcampus.dto.ApproveBookingRequestDto;
import com.smartcampus.dto.BookingCreateRequestDto;
...
public class BookingController {
    ...
    @PostMapping
    public ResponseEntity<?> createBooking(@jakarta.validation.Valid @RequestBody BookingCreateRequestDto request, @AuthenticationPrincipal OAuth2User principal) {
        User user = getAuthenticatedUser(principal);
        if (user == null) return ResponseEntity.status(401).build();
    ...
    public ResponseEntity<?> approveBooking(@PathVariable String id, @RequestBody ApproveBookingRequestDto request) {
        // Require ADMIN role check in production
        String status = request.getStatus() == null || request.getStatus().isBlank() ? "APPROVED" : request.getStatus();
